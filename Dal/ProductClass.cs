...
namespace guanbingking.DAL
{
	...
	public partial class ProductClass
	{
		...
		public guanbingking.Model.ProductClass GetModel(int Id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 Id,ClassName from ProductClass ");
			strSql.Append(" where Id=@Id ");
			SqlParameter[] parameters = {
					new SqlParameter("@Id", SqlDbType.Int,4)			};
			parameters[0].Value = Id;

			guanbingking.Model.ProductClass model=new guanbingking.Model.ProductClass();
		...
		/// 得到一个对象实体
		/// </summary>
		public guanbingking.Model.ProductClass DataRowToModel(DataRow row)
		{
			guanbingking.Model.ProductClass model=new guanbingking.Model.ProductClass();
			if (row != null)
			{
				...
				{
					model.ClassName=int.Parse(row["ClassName"].ToString());
				}
			}
			return model;
		}
